...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    pairs = []
    [(pairs.append([find_closest(restaurant_location(r), centroids), r])) for r in restaurants]
...
def find_centroid(cluster):
    ...
    lat, lon = [], []
    for r in cluster:
        restaurant = restaurant_location(r)
        lat.append(restaurant[0])
        lon.append(restaurant[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            all_rates[name] = user_rating(user, name)
        else:
            all_rates[name] = predictor(r)
    return all_rates
    # END Question 9
